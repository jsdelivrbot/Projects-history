...
export default class EveryNDay extends PureComponent {
  constructor(props) {
    super(props);
    ...
    this.state = {
      ...
      nDayNumber: props.repetition.nDayNumber || 1,
      dateChanged: false,
      showEndDate: false,
    };
  }
  ...
  handlePayload = () => {
    ...
    const newPayload = {
      ...
      nDayNumber: this.state.nDayNumber,
    };
  ...
  handleNDayInput = (e) => {
    if (e.target.value <= 50 && e.target.value > 0) {
      this.setState({ nDayNumber: e.target.value }, () => this.handlePayload());
    }
  }
  ...
  render() {
    return (
      <div>
        ...
        <div>
          <label className={styles.EveryNDay__nDayNumberLabel} htmlFor="nDayNumberInput">
            NDays{' '}
            <input
              id="nDayNumberInput"
              name="nDayNumberInput"
              type="number"
              className={styles.EveryNDay__nDayNumberInput}
              onChange={this.handleNDayInput}
              value={this.state.nDayNumber}
            />
          </label>
        </div>
        <div>
